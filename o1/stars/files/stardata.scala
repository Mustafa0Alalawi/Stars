...
def parseStarInfo(starAsString: String): Star =
  ...
  val xCoord = parts(0).toDouble
  val yCoord = parts(1).toDouble
  val starCoords = new StarCoords(xCoord, yCoord)

  val starID = parts(3).toInt
  ...
  new Star(starID, starCoords, magnitude, name)
